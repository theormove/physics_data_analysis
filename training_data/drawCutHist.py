...
def drawMezonHist(modes):
	...
	for key in modes:
		...
		t.Draw( 'ms_ds'+'>>'+hists[3*i+1].GetName(), modes[key]["cut"] + ' & (gen_ds == 1)' )
		t.Draw( 'ms_ds'+'>>'+hists[3*i+2].GetName(), modes[key]["cut"] + ' & (gen_ds == 0)')		
			
		
		stack = ROOT.THStack(stack_name,"stack_name")
		hists[3*i+2].SetFillColor(ROOT.kGreen+3)
		hists[3*i+2].SetFillStyle(3554)

		
		hists[3*i+1].SetFillColor(ROOT.kGreen+1)
		stack.Add(hists[3*i+1])
		stack.Add(hists[3*i+2])
		stack.Draw()
		legend =  ROOT.TLegend(0.1,0.7,0.48,0.9)
		legend.SetHeader("Candidates")
		legend.AddEntry(hists[3*i + 1].GetName(),"Signal")
		legend.AddEntry(hists[3*i + 2].GetName(),"Background")
		legend.Draw()				
		i += 1
	interrupt = input()
